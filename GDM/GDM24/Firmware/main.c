...
#if defined (VERBOSE)
void sappendf(char *b,float f)
{
    if ( f >= 0.0)
    {
        sprintf(&b[strlen(b)], (const rom far char *)"  %d.%02u",(int)f,((int) (((f)-(int)f) * 100)));
    }
    else
    {
        f = -f;
...
#if defined (VERBOSE)
            sprintf(USB_Out_Buffer, (const rom far char *)" PH=0x%04X",pressS);
            sprintf(&USB_Out_Buffer[strlen(USB_Out_Buffer)], (const rom far char *)" PF=");
            sappendf(USB_Out_Buffer,pressure);
            sprintf(&USB_Out_Buffer[strlen(USB_Out_Buffer)], (const rom far char *)" Temp=");
            sappendf(USB_Out_Buffer,tempC);
            sprintf(&USB_Out_Buffer[strlen(USB_Out_Buffer)], (const rom far char *)" Full=");
            sappendf(USB_Out_Buffer,percentFull);
            sprintf(&USB_Out_Buffer[strlen(USB_Out_Buffer)], (const rom far char *)"\n\r");
#endif
...
static void InitializeSystem(void)
{
    unsigned long bigcounter = 300000;
...
// most every thing is an output.
        ...
        // timer 0 used for a time base
        // 12 Mhz / 256 / 6

        T0CONbits.TMR0ON = 0;   // stop
        T0CONbits.T08BIT = 0;   // 16 bits;
        T0CONbits.T0CS   = 0;   // Fosc/4
        T0CONbits.PSA    = 0;   // prescale on
	...
	UCONbits.USBEN = 0;				// drop  the USB sense
	while(--bigcounter)			// for a short period of time.
		;							// this allows the pc to detect a state change and drop
									// and re-recoginize the device.
...
void ProcessIO(void)
{
    ...
    if(USBUSARTIsTxTrfReady())
    {
        ...
        numBytesToWrite = ParseUsb(USB_In_Buffer,numBytesRead);

#if defined(VERBOSE)
        if ( numBytesRead )
            putUSBUSART(USB_In_Buffer,numBytesRead);
        else if ( strlen(USB_Out_Buffer))
        {
